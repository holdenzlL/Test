...
class SSendingCmd(State):
    ...
    def __on_exit(self, state, event):
        ...
        logging.info("transition:\"%s\"", event.name)
        self.__mUrSm.ClearDoSendFlag()

    def DoTransition(self, state, event):
        logging.info("ON Doing State:\"%s\"", state.name)
        logging.info("transition:\"%s\"", event.name)

        #self.__mNextStatus = "es12"
        self.__mNextStatus = ""
        if self.__mSecNodeUr is not None:
            cmd = self.__mUrSm.GetCmd()
            self.__mSecNodeUr.SendCommand(cmd)
            self.__mUrSm.UpdateUi(cmd)
            self.__mNextStatus = "es12s13"
        else:
            logging.info("Sender module is not established.")
            pass
