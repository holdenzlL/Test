...
class SWaiting4Response(State):
    ...
    def DoTransition(self, state, event):
        ...
        self.__mNextStatus = ""
        response_from_receiver = self.__mSecNodeUr.GetLastResponse()
        self.__mUrSm.UpdateUi(response_from_receiver)
        low_response_from_receiver = response_from_receiver.lower()
        #cmd_from_sender = self.__mUrSm.GetCmd().lower()
        if low_response_from_receiver.strip(): #is not empty
            self.__mNextStatus = "es13s14"

        if self.__mUrSm.IsDoDisconnectFlag():
            self.__mNextStatus = "es13s11"
